...
function passToVue() {
    ...
    console.log(data.books);
    var booksAfterFilter = [];
   
    data.books.forEach(function (everyBook) {
        if (filter(everyBook)) {
//           app.books = [];
            console.log(everyBook);
            booksAfterFilter.push(everyBook);


        }
    })
    app.books = booksAfterFilter;
...
function filter(everyBook) {
    console.log("running filter")
    console.log()
    console.log(everyBook.titulo.toUpperCase());
    var what = false;
    var input = document.getElementById("inputFilter");
    var inputTxt = input.value;

    //    THIS WORKS
    //    if(inputTxt == data.books[1].titulo){
    //        what = true;
    //    }
    // DONT REMOVE WHAT IS UP ^   


    
    if (everyBook.titulo.toUpperCase().includes(inputTxt.toUpperCase()) || inputTxt == "") {
        what = true;
...
//JSON FROM AJAX FOR CLARITY, NOT USED IN JS
//var books = {
//    "books": [{
